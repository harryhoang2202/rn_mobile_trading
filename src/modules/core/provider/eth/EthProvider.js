...
export class EthProvider implements Provider {
    ...
    async getTransactions(wallet): Promise<Object> {
        ...
        if (wallet.type === ASSET_TYPE_COIN) {
            url += `&module=account&action=txlist&address=${wallet.walletAddress}&startblock=0&endblock=999999999&page=1&offset=10&sort=desc`;
        } else {
            url += `&module=account&action=tokentx&contractaddress=${wallet.contract}&address=${wallet.walletAddress}&page=1&offset=30&startblock=0&endblock=999999999&sort=desc`;
        }
        ...
        if (status === 200) {
            const {status: txStatus, result} = data;
            if (txStatus === '1') {
                for (let i = 0; i < result.length; i++) {
                    ...
                    let explore =
                        configProperties.eth.explore + `tx/${item.hash}`;
                    switch (wallet.chain) {
                        case 'POLYGON':
                            explore =
                                configProperties.polygon.explore +
                                `tx/${item.hash}`;
                            break;
                        case 'BSC':
                            explore =
                                configProperties.bsc.explore +
                                `tx/${item.hash}`;
                            break;
                        case 'ARB':
                            explore =
                                configProperties.arb.explore +
                                `tx/${item.hash}`;
                            break;
                        case 'BTTC':
                            explore =
                                configProperties.bttc.explore +
                                `tx/${item.hash}`;
                            break;
                        default:
                            break;
